...
def pairwise_similarity(outputs,temperature=0.5, multi_gpu=False, adv_type='None'):
    ...
    '''
    print("Checking on distributed ", dist.get_world_size())
    if multi_gpu and adv_type=='None':
        ...
        gather_t_1 = [torch.empty_like(outputs_1) for i in range(multi_gpu)] #dist.get_world_size()
        gather_t_1 = distops.all_gather(gather_t_1, outputs_1)

        gather_t_2 = [torch.empty_like(outputs_2) for i in range(multi_gpu)] #dist.get_world_size()
        ...
        gather_t_3 = [torch.empty_like(outputs_3) for i in range(multi_gpu)] #dist.get_world_size()
